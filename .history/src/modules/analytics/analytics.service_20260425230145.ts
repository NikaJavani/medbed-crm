...
export const getTechnicianPerformance = async () => {
  ...
  return technicians.map((tech) => {
    ...
    const resolvedWithTime = tech.serviceReports.filter(
      (r) => r.ticket.resolvedAt && r.ticket.createdAt
    )
      ...
      resolvedWithTime.length > 0
        ? (
            resolvedWithTime.reduce((sum, r) => {
              const created = new Date(r.ticket.createdAt).getTime()
              const resolved = new Date(r.ticket.resolvedAt!).getTime()
